...
const LoungeReligionId = () => {
  const dispatch = useAppDispatch();
  let { religion_id } = useParams();
  ...
  const [listReligionsPostActive, setListReligionsPostActive] = useState([]);
  ...
  useEffect(() => {
    if (religion_id) {
      dispatch(religionID(religion_id));
    }
  ...
  const handleFirstAndSave = async (tab) => {
    let getReligionsPostActive = "";
    if (tab === "tab1") {
      getReligionsPostActive = await axiosClients.get(
        `/post/${religion_id}/list`,
        {
    ...
    if (tab === "tab2") {
      getReligionsPostActive = await axiosClients.get(
        `/post/${religion_id}/list?type=COMMUNICATION`,
        {
          headers: authHeaderAndAccount(),
        }
      );
    ...
    if (tab === "tab4") {
      getReligionsPostActive = await axiosClients.get(
        `/post/${religion_id}/list?type=POLL`,
        {
          headers: authHeaderAndAccount(),
        }
      );
    }
    console.log(getReligionsPostActive);
    if (tab === "tab5") {
      getReligionsPostActive = await axiosClients.get(
        `/post/${religion_id}/list?type=EVENT`,
        {
          headers: authHeaderAndAccount(),
        }
      );
    ...
    setListReligionsPostActive(getReligionsPostActive.data.items);

    if (getReligionsPostActive.data.next_page) {
      setPageKey(`page_key=${getReligionsPostActive.data.next_page}`);
      setLastPage(false);
    } else {
      setLastPage(true);
    }
  ...
  const fetchImages = async () => {
    if (!lastPage) {
      ...
      if (activeTab === "tab2") {
        getReligionsPostActive = await axiosClients.get(
          `/post/${religion_id}/list?type=COMMUNICATION&${pageKey}`,
          {
            headers: authHeaderAndAccount(),
          }
        );
      ...
      if (activeTab === "tab4") {
        getReligionsPostActive = await axiosClients.get(
          `/post/${religion_id}/list?type=POLL&${pageKey}`,
          {
            headers: authHeaderAndAccount(),
          }
        );
      ...
      if (activeTab === "tab5") {
        getReligionsPostActive = await axiosClients.get(
          `/post/${religion_id}/list?type=EVENT&${pageKey}`,
          {
            headers: authHeaderAndAccount(),
          }
        );
      ...
      setListReligionsPostActive([
        ...listReligionsPostActive,
        ...getReligionsPostActive.data.items,
      ]);
      console.log(getReligionsPostActive);
      if (getReligionsPostActive.data.next_page) {
        setPageKey(`page_key=${getReligionsPostActive.data.next_page}`);
  ...
  return (
    <div id="main">
      ...
      <InfiniteScroll
        dataLength={listReligionsPostActive.length}
        next={fetchImages}
        hasMore={true}
      ...
      >
        ...
        <div className="content">
          <div className="container no-padding-lr">
            ...
            <div>
              <BoxLeft religionID={religion_id} />
            </div>
            ...
            <div className="div503">
              ...
              <div className="updatesWrapper">
                ...
                <div id="scrollableDiv">
                  {!isEmpty(listReligionsPostActive)
                    ? listReligionsPostActive.map((data, index) => (
                        <div key={index}>
            ...
            <BoxRight religionID={religion_id} />
...
export default LoungeReligionId;
