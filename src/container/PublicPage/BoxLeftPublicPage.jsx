...
const BoxLeftPublicPage = () => {
  const [listJoinedReligions, setListJoinedReligions] = useState([]);
  const [listTouring, setListTouring] = useState([]);
  const [getFeatureReligion, setGetFeatureReligion] = useState({});
  const [diffDate, setDiffDate] = useState("");

  useEffect(() => {
    (async () => {
      if (localStorage.getItem(STORAGE_KEY.IS_LOGIN)) {
        const data = await axiosClients.get(apiGets.getJoinedReligions, {
          headers: authHeaderAndAccount(),
        });

        if (data.message === "ok") {
          setListJoinedReligions(data.data);
        }
      }
    })();
  ...
  useEffect(() => {
    (async () => {
      if (localStorage.getItem(STORAGE_KEY.IS_LOGIN)) {
        const touring = await axiosClients.get(apiGets.getFollowedReligions, {
          headers: authHeaderAndAccount(),
        });
  ...
  useEffect(() => {
    (async () => {
      const feature = await axiosNoAuth.get(apiGets.getFeatureReligion);
      ...
      if (feature.message === "ok") {
        setGetFeatureReligion(feature.data);
      }
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const handleJoinedReligion = (item) => {
    window.location.href = `/lounge/${item.religion_id}`;
  };
    ...
    <div id="sidebarleft" className="div306 leftSideBar">
      <div className="sibar-inner-wrapper">
          ...
          <div className="card-body">
            <div className="author">
              <img
                ...
                onClick={() => handleJoinedReligion(getFeatureReligion)}
                src={
                  !isEmpty(getFeatureReligion.founder_picture_url_full)
                    ? getFeatureReligion.founder_picture_url_full
              ...
              <p>
                <a
                  href={`lounge/${getFeatureReligion.religion_id}`}
                  className="bright-blue"
                >
                  {getFeatureReligion.name}
                </a>
                <br />{" "}
                <span className="fs--12">
                  {getFeatureReligion.mantra}
                  <br />{" "}
                  <span className="pr--5">
                    {" "}
                    {getFeatureReligion.affiliate_count} Affiliates
                  </span>{" "}
                  {diffDate}
                  old
                </span>
            ...
            <p className="fs--12">{getFeatureReligion.major_orientation}</p>
          </div>
        </div>
        {/* end of card */}
        {/* start of card */}
        <div className="card mt--5 ujoined">
          ...
          <div className="card-body">
            {/* start of group */}
            {!isEmpty(listJoinedReligions)
              ? listJoinedReligions.map((item, index) => {
                  return (
                    <div
                      className="author pointerA"
                      onClick={() => handleJoinedReligion(item)}
                    >
        ...
        <div className="card mt--5 impt-weight">
          ...
          <div className="rateBarWrapper1">
            ...
            <p className="fs--11">
              The religion’s membership composition and how members and leaders
