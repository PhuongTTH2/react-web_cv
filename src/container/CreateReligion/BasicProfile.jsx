...
const BasicProfile = ({ handleStep, handleReligionID }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      let addOther = [...getReligionInfluences.data, "Other"];
      const getCheck = [];
  ...
  const onSubmit = async (inputs) => {
    ...
    const res = await axiosClientsFormData({
      method: "post",
      url: apiPosts.createReligion,
      headers: authHeaderAndAccount(),
      data: bodyFormData,
    });
    if (res.message === "ok") {
      handleStep(3);
      handleReligionID(res.data.religion_id);
    } else {
    }
  };
