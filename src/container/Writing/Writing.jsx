...
const Page = () => {
  ...
  useEffect(() => {
    (async () => {
      if (bookCallUseEffect) {
        let data = await axiosClients.get(`/religion/${religion_id}/books`, {
        ...
        if (!isEmpty(data.data)) {
          let dataFirst = await axiosClients.get(
            `/book/${data.data[0].book_id}/menu-contents/${data.data[0].religion_id}`,
            {
              headers: authHeaderAndAccount(),
            }
          );
    ...
    (async () => {
      if (chapterCallUseEffect) {
        let data = await axiosClients.get(
          `/book/${bookId}/menu-contents/${religion_id}`,
          {
            headers: authHeaderAndAccount(),
          }
        );
  ...
  const handleBookSelect = async (book, index) => {
    ...
    let data = await axiosClients.get(
      `/book/${book.book_id}/menu-contents/${book.religion_id}`,
      {
        headers: authHeaderAndAccount(),
      }
    );
  ...
  const handleEditTopic = async (chapterId, topicId) => {
    const res = await axiosClients.get(
      `/book/${bookId}/${chapterId}/topic/${topicId}/${religion_id}`,
      {
        headers: authHeaderAndAccount(),
      }
    );
